...
// For GitHub Pages hosting, update this to your backend URL (e.g., https://your-app.onrender.com/api)
const API_BASE_URL = '/api';

// Try to detect if we're on GitHub Pages and automatically adjust
function getAPIBaseURL() {
    // If running locally (localhost or 127.0.0.1), use relative path
    if (window.location.hostname === 'localhost' || window.location.hostname === '127.0.0.1') {
        return '/api';
    }
    
    // If running on GitHub Pages, update this to your backend server URL
    if (window.location.hostname.includes('github.io')) {
        // Replace with your actual backend URL
        // Example: return 'https://fitness-api.onrender.com/api';
        // For now, we'll use the relative path and let you configure it
        return '/api';
    }
...
const CONFIGURED_API_URL = getAPIBaseURL();
...
async function loadTrainings() {
    try {
        const response = await fetch(`${CONFIGURED_API_URL}/trainings`);
        ...
        showAlert('Error loading training plans: ' + error.message, 'error');
    }
}
...
async function applyTrainingFilters() {
    ...
    try {
        let url = `${CONFIGURED_API_URL}/trainings?`;
...
async function loadDiets() {
    try {
        const response = await fetch(`${CONFIGURED_API_URL}/diets`);
        ...
        showAlert('Error loading diet plans: ' + error.message, 'error');
    }
}
...
async function applyDietFilters() {
    ...
    try {
        let url = `${CONFIGURED_API_URL}/diets?`;
...
async function displayMyPlans() {
    ...
    try {
        ...
        if (selectedPlans.trainings.length > 0) {
            ...
            const trainingResponse = await fetch(`${CONFIGURED_API_URL}/trainings?ids=${trainingIds}`);
        ...
        if (selectedPlans.diets.length > 0) {
            ...
            const dietResponse = await fetch(`${CONFIGURED_API_URL}/diets?ids=${dietIds}`);
            if (dietResponse.ok) {
...
async function showPlanModal(type, planId) {
    ...
    try {
        ...
        const response = await fetch(`${CONFIGURED_API_URL}/${endpoint}/${planId}`);
        ...
        showAlert('Error loading plan details: ' + error.message, 'error');
    }
}
